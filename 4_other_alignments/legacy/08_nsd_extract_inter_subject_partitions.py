"""
Computes inter-subject alignment across NSD ROIs under content-based stimulus partitions.

This legacy script:
- Pairs beta responses from ROI_x (subject_x) and ROI_y (subject_y) across shared stimuli.
- Supports multiple similarity metrics (e.g., RSA, unbiased CKA).
- Allows filtering by stimulus content (e.g., motion, animal, person).
- Optionally computes diagonal alignments only (ROI_x == ROI_y).

Results are stored in a flat dataframe with subject/ROI/partition identifiers.

Requires: NSD betas, COCO-style object annotations, and motion labels.
"""
